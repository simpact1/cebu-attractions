...
import { googleMapsSearchUrl, type CebuGuideItem } from "./cebuPlacesData";
...
function MapFitAndFocus({
  ...
  const map = useMap();
  const markerBoundsKey = useMemo(() => markers.map((m) => `${m.id}`).join("|"), [markers]);
  ...
  useEffect(() => {
    ...
    if (!m) return;
    map.flyTo([m.lat, m.lng], 16, { duration: 0.45 });
    ...
    () =>
      items
        ...
        .map((i) => ({
          ...
          lng: i.mapPin.lng,
          title: i.title,
          mapsQuery: i.mapsQuery,
          mapPopupLink: i.mapPopupLink,
        })),
    [items],
  );
            ...
            <Popup>
              <div className="pg-cluster-popup">
                ...
                {m.mapPopupLink ? (
                  ...
                  <a
                    className="pg-cluster-popup-link"
                    href={googleMapsSearchUrl(m.mapsQuery)}
                  >
                    Google 지도에서 열기
                  </a>
                ) : null}
